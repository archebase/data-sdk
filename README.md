...
```swift
dependencies: [
    .package(url: "https://github.com/archebase/data-sdk.git", from: "0.1.1")
]
```
...
`listObjects(_:authorizationHeader:)` 使用用户 access JWT 调用 Data Gateway object service，返回当前用户可见的 verified 逻辑对象。它不使用上传 `api_key`。
